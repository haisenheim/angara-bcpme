...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('regional.dashboard') }}">Angara</a></li>
...
@section('page-header')
    <div>
        ...
        <p class="text-body-secondary mb-0 mt-1">Périmètre représentation</p>
    </div>
@endsection
        ...
        <div class="card-body py-3">
            ...
            <div class="row g-3 align-items-end mt-1 pt-2 border-top border-light-subtle">
                ...
                <div class="col-12 col-md-auto ms-md-auto pt-1 pt-md-0">
                    ...
                    <div class="dropdown">
                        <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" id="regional-prospects-actions" data-bs-toggle="dropdown" aria-expanded="false">Actions</button>
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="regional-prospects-actions">
...
document.addEventListener('DOMContentLoaded', function() {
    const baseUrl = "{{ url('regional/entreprises') }}";
    const paginatedUrl = "{{ route('regional.prospects.paginated') }}";
    const statsUrl = "{{ route('regional.prospects.stats') }}";
    const filterOptionsUrl = "{{ route('regional.prospects.filter-options') }}";
    const exportUrl = "{{ route('regional.prospects.export') }}";
    ...
    fetch(filterOptionsUrl)
        ...
        .then(data => {
            (data.regions || []).forEach(r => {
                ...
                document.getElementById('filter-region').appendChild(opt);
            });
            (data.formes || []).forEach(f => {
                const opt = document.createElement('option');
                opt.value = f.id;
                opt.textContent = f.name;
                document.getElementById('filter-forme').appendChild(opt);
            });
            (data.agences || []).forEach(a => {
                const opt = document.createElement('option');
                opt.value = a.id;
                opt.textContent = a.name;
                document.getElementById('filter-agence').appendChild(opt);
            });
            (data.gestionnaires || []).forEach(u => {
                const opt = document.createElement('option');
                opt.value = u.id;
                opt.textContent = u.name;
                document.getElementById('filter-gestionnaire').appendChild(opt);
            });
        })
    ...
    const table = new AngaraTable(document.getElementById('prospectsTable'), {
        ...
        pageLength: 15,
        searchDelay: 350,
        filters: { region_id: 'filter-region', taille: 'filter-taille', forme_id: 'filter-forme', caractere: 'filter-caractere', agence_id: 'filter-agence', gestionnaire_id: 'filter-gestionnaire', submission: 'filter-submission' },
        columns: [
