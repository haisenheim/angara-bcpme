...
@section('page-header')
    <div>
        ...
        <p class="text-body-secondary mb-0 mt-1">Consultez l'ensemble des programmes de l'agence</p>
    </div>
@endsection
...
document.addEventListener('DOMContentLoaded', function() {
    const baseUrl = "{{ url($rp.'/programmes') }}";
    const paginatedUrl = "{{ route($rp.'.programmes.paginated') }}";
    const statsUrl = "{{ route($rp.'.programmes.stats') }}";
    const filterOptionsUrl = "{{ route($rp.'.programmes.filter-options') }}";
