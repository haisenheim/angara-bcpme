...
@php
    ...
    $analysteCanEdit = $isAssignedAr && ! $submittedByAnalyste;
@endphp
    ...
    <div class="card-body">
        ...
        @if($isRerx && $submittedByAnalyste && ! $submittedToDirection)
            <div class="mb-0">
                ...
                <form method="post" action="{{ route('rerx.dossiers.soumettre-direction', $dossier->token) }}" onsubmit="return confirm('Transmettre ce dossier au directeur général et au directeur général adjoint ?');">
                    @csrf
                    <button type="submit" class="btn btn-dark">Soumettre à la direction (DG &amp; DGA)</button>
                </form>
                <p class="small text-muted mt-2 mb-0">La transmission enregistre la date, l’heure et votre identité ; le dossier devient consultable par la direction.</p>
            </div>
