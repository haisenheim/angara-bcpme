@extends('Layouts.chef_filiere')

@section('title', 'Dossiers d\'instruction — en cours')

@section('breadcrumb')
<nav aria-label="breadcrumb">
    <ol class="breadcrumb mb-0">
        <li class="breadcrumb-item"><a href="{{ route('chef-filiere.dashboard') }}">Tableau de bord</a></li>
        <li class="breadcrumb-item active" aria-current="page">Instruction — en cours</li>
    </ol>
</nav>
@endsection

@section('page-header')
<div>
    <h5 class="page-title mb-0">Dossiers d'instruction</h5>
    <p class="text-body-secondary mb-0 mt-1 small">Suivi en lecture seule des dossiers créés après validation.</p>
</div>
@endsection

@section('content')
<div class="cf-page">
    <div class="cf-hero mb-4">
        <h1 class="cf-hero__title">En cours d'instruction</h1>
        <p class="cf-hero__lead">Dossiers instruits par les équipes (gestionnaire, analyste) pour votre agence.</p>
        <div class="cf-hero__meta">
            <span class="cf-kpi"><span class="text-muted fw-normal">Dossiers</span> <span class="cf-kpi__val">{{ $items->count() }}</span></span>
        </div>
    </div>

    <div class="cf-panel">
        <div class="cf-panel__toolbar">
            <p class="cf-panel__toolbar-label mb-0">Suivi</p>
        </div>
        <div class="cf-table-wrap">
            <table class="table cf-table mb-0 align-middle">
                <thead>
                    <tr>
                        <th scope="col">Client</th>
                        <th scope="col">Programme</th>
                        <th scope="col">Gestionnaire</th>
                        <th scope="col">Analyste</th>
                        <th scope="col">Statut</th>
                        <th scope="col" class="text-end">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($items as $dossier)
                        @php $st = $dossier->status; @endphp
                        <tr>
                            <td class="fw-semibold text-dark">{{ $dossier->entreprise?->name ?? '—' }}</td>
                            <td>{{ $dossier->programme?->name ?? '—' }}</td>
                            <td>{{ $dossier->gestionnaire?->name ?? '—' }}</td>
                            <td>{{ $dossier->analyste?->name ?? '—' }}</td>
                            <td><span class="badge rounded-pill bg-secondary bg-opacity-75">{{ $st['name'] ?? '—' }}</span></td>
                            <td class="text-end text-nowrap">
                                <a href="{{ route('chef-filiere.instructions.dossier.show', $dossier->token) }}" class="btn btn-sm btn-outline-primary">Détail</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="cf-empty border-0">
                                <i class="cf-empty__icon pli-folder" aria-hidden="true"></i>
                                Aucun dossier d'instruction pour cette agence.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
</div>
@endsection
