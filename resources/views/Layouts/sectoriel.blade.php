...
@section('top')
<?php
$secteur = \Illuminate\Support\Facades\Session::get('secteur');
?>
    <div>
        <div class="d-flex gap-3">
            <div>
                <span><span class="badge bg-white text-dark fs-6">{{ $secteur?->name }} - {{ $secteur?->agence?->name }} </span> </span>
            </div>
            <div>
                <span>-</span>
                <strong><span class="badge bg-white text-dark fs-6">Responsable sectoriel</span></strong>
            </div>
        </div>
    </div>
...
<div class="mainnav__categoriy py-3 mb-0">
    <ul class="mainnav__menu nav flex-column gap-2">
       <li class="nav-item">
           <a href="{{ route('sectoriel.dashboard') }}" class="nav-link mininav-toggle {{ $active==1?'active':'' }}"><i class="demo-pli-home fs-3 me-2"></i>
